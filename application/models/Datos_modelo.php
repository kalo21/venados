...
class Datos_modelo extends CI_Model{
    ...
    public function obtenerDatos($id) {
        ...
        $this->db->join('vendedores','clientes.id_usuario = vendedores.id_usuario');
            ...
            $this->db->select('empresas.logotipo as imagen,clientes.nombre,CONCAT(clientes.apellidopaterno," ",clientes.apellidomaterno) as apellido,clientes.saldo,clientes.id,clientes.telefono,usuarios.correo');
            $this->db->from('clientes');
            $this->db->join('usuarios','clientes.id_usuario = usuarios.id');
