		...
		<div class="container-fluid">
			<div class="row">
				<blockquote style="border-left: 5px solid #2f4159">
					<h1 class="text-justify">Carreras</h1>
				</blockquote>
			</div>
			<div class="row">
				<div class="col-md-4">
			...
			<div class="row">
				<div class="box box-primary">
					<div class="box-header with-border">
						<h3 class="box-title"><i class="fa fa-graduation-cap"></i> Listado de carreras</h3>
					...
					<div class="box-body">
						<div class="table-responsive">
							<table id="tablaCarreras" class="table table-striped no-margin" style="width: 100%">
								<thead>
									<tr>
										<th>ID</th>
										<th>Nombre</th>
							...
							<button type="button" class="btn btn-primary btn-lg" id="agregarCarrera" style="background-color: #264d78">Agregar carrera</button>
						</div>
					</div>
				</div>
			</div>
...
<script>
	...
	console.log(x);
	//Declaro la tabla 
	var tabla = insertarPaginado("tablaCarreras",10,true);
	...
	function obtenerDatos(estatus){
		$.ajax({
			url:base_url+'index.php/Administrador/obtenerCarreras/'+estatus,
			type: 'POST',
			beforeSend: function(){
				avisoCargando.open();
			},
			success:function(respuesta){
				...
				$.each(respuesta,function(index, item){
					...
					var fila = tabla.row.add([
							...
							item['abreviatura'],
							ac,
							"<a href='#' class = 'editarCarrera' data-id="+item['id']+"><span class = 'fa fa-edit' style='font-size: 2.3em; color: #264d78'></span></a>&nbsp;&nbsp;&nbsp;&nbsp;"+varStatus+'&nbsp;&nbsp;&nbsp;&nbsp;'
						]).draw(false).node();
	...
	$('#contenidoTabla').delegate('.editarCarrera', 'click', function(){
		var dodo = $(this); // Acceddr al contenido de la etiqueta 'a'
		var actualid = dodo.data('id');
		BootstrapDialog.show({
			title: 'Editar Carrera', // Aqui se pone el titulo
			...
			data: {
				'pageToLoad': base_url+'index.php/administrador/formularioCarreras/'+actualid
			},
			buttons: [{
					label: 'Cancelar',
					cssClass: 'btn-danger',
				...
				{ //agrega los botones del modal
					...
					action: function(dialogItself){
						...
						var abreviaturaVieja = $('#txtabreviatura').data('info');
						...
						if (descripcion != descripcionVieja || abreviatura != abreviaturaVieja) {
							//Comparamos si se hizo un cambio con la informacion que ya se tenia
							BootstrapDialog.confirm({
								title: 'informacion',
								message: 'La siguiente carrera sera modificada ¿Desea continuar?',
								...
								callback: function(result){
									if (result) {
										...
										$.ajax({
											url:base_url+"index.php/Administrador/editarCarrera",
											type:"POST",
											data:formData,
											cache:false,
											contentType:false,
											...
											success:function(respuesta){
												switch(parseInt(respuesta)){
													...
													case 0:
                                            			...
                                            			setTimeout(function(){
                                            				window.location.href = base_url+'index.php/Administrador/carreras'
                                            			},1000);
                                            			$('#txtnombre').val('');
                                            		break;
                                            		case 2:
	...
	$('#contenidoTabla').delegate('.cambiarEstatus', 'click', function(){
		...
		console.log(actualado);
		var msg = '';
		if (actualado == '1') {
			actualado = 0;
			msg = '<br><br> <strong>Nota:</strong> A dar de baja una carrera, no se podrá elegir al momento de solicitar un prestamo';
		}else{
			actualado = 1;
		}
		BootstrapDialog.confirm({
			title: 'Advertencia',
			message: 'Se cambiará el estatus de la carrera ¿Desea continuar?'+msg,
			...
			callback: function(result){
				if (result) {
					$.ajax({
						url:base_url+'index.php/Administrador/cambiarEstatusCarrera/',
						type: 'POST',
						data:{
							id: actualid,
							estatus: actualado
	...
	$('#agregarCarrera').click(function(){
		BootstrapDialog.show({
			title: 'Nueva carrera', // Aquí se pone el titulo
			...
			data: {
				'pageToLoad': base_url+'index.php/administrador/formularioCarreras'
			},
			buttons:[{ // Agregamos los botones
				...
				{ // Agrega los botones del modal
					...
					action: function(dialogItself){
						...
						var formData = new FormData($('#formulario')[0]);
						$.ajax({
							url:base_url+"index.php/Administrador/agregarCarrera",
							type: 'POST',
							data:formData,
							cache:false,
							contentType: false,
